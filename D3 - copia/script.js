...
Promise.all([
  ...
  function actualizarMapa(contaminanteSeleccionado) {
    
// A. Filtrar o agrupar los datos
    let datosFiltrados = [];
    if (contaminanteSeleccionado === "Promedio") {
      const grupos = d3.group(datosCompletos, d => d["AQ Zone Id"].trim().toUpperCase());
      ...
      datosFiltrados = datosCompletos.filter(d => d["Air Pollutant"] === contaminanteSeleccionado);
    }
    ...
    const featuresFiltradas = geojson.features.filter(f => {
...
const svgGrafico = d3.select("#grafico-barras")
    .append("svg").attr("width", 1100).attr("height", 800);

function actualizarGrafico(datosParaGrafico) {
    svgGrafico.selectAll("*").remove();
 const top = datosParaGrafico.sort((a, b) => b.I - a.I).slice(0, 50);

    const margin = {top: 20, right: 0, bottom: 40, left: 290}; // Espacio para nombres
...
const nuevasBarras = barras.join(
    enter => enter.append("rect")
        ...
        .attr("pointer-events", "all") // Asegura que el ratón detecte la barra
);
